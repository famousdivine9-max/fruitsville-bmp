...
const NAV = [
  ...
  { to: '/admin/reviews', label: 'Reviews', minRole: 'manager' },
...
export default function AdminLayout() {
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `rounded-lg px-3 py-2 text-sm font-medium ${
              isActive ? 'bg-white text-ink' : 'text-white/85 hover:bg-white/10'
            }`
          }
        >
          {n.label}
  ...
  return (
    <div className="min-h-screen bg-gray-50 md:flex">
      <aside
        className={`${open ? 'block' : 'hidden'} bg-ink p-4 text-white md:sticky md:top-0 md:block md:h-screen md:w-60 md:shrink-0`}
      >
        ...
        <div className="mt-6 border-t border-white/15 pt-4 text-xs text-white/70">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button onClick={signOut} className="btn btn-sm bg-white text-ink hover:bg-cream-dark">Sign out</button>
          </div>
        </div>
      </aside>
